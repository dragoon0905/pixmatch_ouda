...
class Eval():
    ...
    def Harmonic_Intersection_over_Union(self, out_16_13=False):
        MIoU = np.diag(self.confusion_matrix) / (
            np.sum(self.confusion_matrix, axis=1) + np.sum(self.confusion_matrix, axis=0) -
            np.diag(self.confusion_matrix))
        if self.synthia:
        ...
        if out_16_13:
            ...
            return MIoU_16, MIoU_13
        # MIoU = np.nanmean(MIoU[:self.ignore_index])
        known_MIoU = np.nanmean(MIoU[:-1])
        unknown_MIOU = np.nanmean(MIoU[-1])
        H_MIoU = 2 * known_MIoU * unknown_MIOU / (known_MIoU + unknown_MIOU)
